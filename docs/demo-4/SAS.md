...
## OptiVision Form Analysis

### POST /api/vision/analyze
**Service Name:** Vision Form Analysis Ingestion Service

**Description:**
Accepts client-side extracted pose landmark frames for a recorded exercise, stores the raw frame payload into Azure Blob Storage (`optivision-payloads` / `jobs`), registers a new analysis job in PostgreSQL with status `Pending`, and dispatches a claim-check message onto Azure Service Bus queue (`cv-jobs-queue`) for asynchronous processing by computer vision worker nodes.

**Inputs:**
- `exercise`: string - Name or identifier of the exercise being analyzed (e.g., `"Squat"`).
- `view`: string - Camera viewpoint angle (e.g., `"side"`, `"front"`).
- `frames`: array of `VisionFrame` - Sequential video frames containing 3D skeletal landmarks:
	- `timestamp`: double - Video timestamp offset in seconds.
	- `landmarks`: array of `VisionLandmark` - Normalized 3D skeletal coordinates:
		- `x`: double - Normalized horizontal coordinate.
		- `y`: double - Normalized vertical coordinate.
		- `z`: double - Estimated depth coordinate.
- `access_token` cookie: string - HTTP-only cookie passed by the browser identifying the current authenticated user.

**Outputs:**
- `200 OK` with JSON object:
	- `jobId`: string - Unique identifier assigned to the created analysis job (e.g., `"job_a1b2c3d4..."`).
- `400 Bad Request` if `exercise` is missing or the request payload is invalid.
- `401 Unauthorized` if the `access_token` cookie or valid user claim is missing.
- `429 Too Many Requests` if the AI rate limit (20 requests per minute) is exceeded.

**Usage / Interaction Rules:**
- Clients must send a POST request to `/api/vision/analyze` with JSON data and the `access_token` cookie attached.
- Enforces rate limiting under the AI policy (`RateLimitPolicies.Ai`).
- Implements the Claim-Check architectural pattern: the large landmark frame payload is uploaded to Azure Blob Storage, and a reference message containing the metadata and `blobUrl` is dispatched to the Azure Service Bus queue.
	...
	"frames": [
		{
			"timestamp": 0.033,
			"landmarks": [
				{ "x": 0.52, "y": 0.31, "z": -0.15 },
				{ "x": 0.54, "y": 0.62, "z": -0.10 }
			]
		},
		{
			"timestamp": 0.066,
			"landmarks": [
				{ "x": 0.53, "y": 0.32, "z": -0.14 },
				{ "x": 0.55, "y": 0.64, "z": -0.09 }
			]
		}
	]
}
...
```json
{
	"jobId": "job_e9b46995079a4087b3be17ff14316d9b"
}
```

---

### POST /api/vision/worker-result
**Service Name:** Vision Worker Callback Service

**Description:**
Internal callback endpoint invoked by computer vision worker nodes upon completing landmark analysis and biomechanical anomaly detection. If successful, it invokes Google Gemini 1.5 Flash via `IGeminiClient` to generate an actionable 2-sentence coaching tip, persists detected anomalies and coach advice in PostgreSQL, and marks the job status as `Completed` (or `Failed`).

**Inputs:**
- `jobId`: string - Unique identifier of the analysis job being reported.
- `success`: boolean - Indicates whether computer vision processing succeeded.
- `detected_anomalies`: array of objects or strings - Biomechanical faults detected during the exercise:
	- Object format: `{ "error": string, "severity": double }`
	- String format: `string` (e.g., `"Knees caving inward"`)

**Outputs:**
- `200 OK` with JSON object:
	- `success`: boolean - `true` indicating the callback was successfully processed.
- `400 Bad Request` if `jobId` is missing or the payload is null.
- `404 Not Found` if no job matching `jobId` is found in the database.

**Usage / Interaction Rules:**
- Invoked by the internal Python worker service. This endpoint allows anonymous access (`[AllowAnonymous]`) for decoupled service-to-service communication.
- The anomaly parser supports both raw string arrays and structured objects with error names and severity/confidence scores.
- When `success` is `true`, Gemini generates a concise 2-sentence coaching critique based on the specific exercise and detected faults before updating the job.
...
```json
{
	...
	"detected_anomalies": [
		...
		{
			"error": "Insufficient squat depth",
			"severity": 0.60
		}
	]
}
...
### GET /api/vision/result/{jobId}
**Service Name:** Vision Job Result Query Service

**Description:**
Retrieves the execution status and analysis results (coaching summary, overall movement form score, formatted list of detected issues) for a specific vision analysis job. Polled by the frontend application while the asynchronous processing pipeline executes.

**Inputs:**
- `jobId`: string - Path parameter specifying the unique identifier of the vision analysis job.
- `access_token` cookie: string - HTTP-only cookie passed by the browser identifying the current user.

**Outputs:**
- `200 OK` with `VisionResultResponse`:
	- `status`: string - Current job lifecycle status (`"processing"`, `"completed"`, or `"failed"`).
	- `coach_summary`: string | null - 2-sentence personalized coaching tip from Gemini (null while processing).
	- `score`: integer - Calculated movement score (0-100) based on detected anomalies.
	- `issues`: array of string | null - Title-cased, human-readable names of detected movement faults (null while processing).
- `400 Bad Request` if the `jobId` path parameter is empty.
- `401 Unauthorized` if the `access_token` cookie is missing or invalid.
- `404 Not Found` if the job does not exist.

**Usage / Interaction Rules:**
- Clients send a GET request to `/api/vision/result/{jobId}` with the `access_token` cookie attached.
- Designed for polling every 1-2 seconds after set submission until status transitions away from `"processing"`.
- Requires user authentication; returns `401 Unauthorized` if unauthenticated.

**Example Request:**
```http
GET /api/vision/result/job_e9b46995079a4087b3be17ff14316d9b HTTP/1.1
```
...
{
	"status": "completed",
	"coach_summary": "Push your knees outward in line with your toes to prevent valgus collapse during the ascent. Focus on maintaining chest elevation throughout the bottom of the squat.",
	"score": 78,
	"issues": [
		"Knees Caving Inward",
