  ...
  return (
    <div className={className}>
      <div className="rounded-xl border border-border bg-surface p-3 shadow-sm sm:p-4">
        ...
        <div className="grid grid-cols-7 gap-y-1.5 text-center text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-muted-foreground sm:text-[0.72rem] sm:tracking-[0.18em]">
          {WEEKDAY_LABELS.map((label) => (
            <div key={label}>{label}</div>
          ))}
        </div>

        <div className="mt-1.5 grid grid-cols-7 justify-items-center gap-x-1 gap-y-1.5 text-center sm:mt-2 sm:gap-y-2">
          {gridDays.map((day) => {
            ...
            return (
              <div key={day.cellKey} className="flex justify-center">
                <button
                  type="button"
                  className={[
